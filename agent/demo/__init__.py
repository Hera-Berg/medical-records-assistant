...
#: The demo's own ``config.toml``. **No ``[models.vlm]``**, deliberately.
#:
#: It used to carry the template's placeholder MagicDNS name, which made
...
#: connection to a model, so the table is absent and the absence is explained
#: where someone hits it.
#:
#: ``[models.asr]`` stays: speech runs locally, so it needs no endpoint and no
#: credential, and phase 6 can be exercised against this vault as it is.
DEMO_CONFIG = """\
...
# box being asleep. Run extraction against your real vault, or paste your
# [models.vlm] table in below — see MODELS.md.
...
Two honest gaps. The voice note is a tone rather than speech — nothing here
synthesises a voice — so transcribing it will correctly produce nothing. And
this vault carries no `[models.vlm]` endpoint: a demo seeds a record, not a
connection to a model. Point `--vault` at your real vault to extract, or add an
endpoint to `config.toml` here.
...
def seed(root: str | Path, as_of: datetime | None = None) -> DemoReport:
    ...
    *as_of* anchors the scenario: every document date is an offset back from it,
    ...
    (root / CONFIG_FILENAME).write_text(DEMO_CONFIG, encoding="utf-8")
